    program hisread
!---------------------------------------------------------------------
!   read lammps trajectory file and wirte the trajectory in XYZ format
!   written by Francesco Tavanti, April 2020
!   CNR-NANO S3, Modena, Italy
!
!   USAGE:
!   build the fortran executable ==> gfortran lammpstraj2XYZ.f90
!   then ./a.out
!   insert the name of input lammps file
!   insert the name of output file in XYZ format
...
character*1 option
character*4 atname
character*80 lammpsfile,xyzfile
...
write(*,*)''
write(*,*)'*********************'
write(*,*)'  This script reads a lammps trajectory file and writes the trajectory in XYZ format'
write(*,*)'  The trajectory could come both from serial and from parallel simulations'
write(*,*)'  written by Francesco Tavanti, April 2020'
...
write(12,'(a2,3f12.4)')atype(i),(pos(k,i),k=1,3)
exit
